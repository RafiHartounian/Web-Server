...
TEST_F(RequestParserTest, GoodRequest){
	...
	boost::tie(result,boost::tuples::ignore) = parser.parse(request_, input, input + strlen(input));
	EXPECT_TRUE(result);
}
...
TEST_F(RequestParserTest, ParserValidRequest) {
  	...
  	char input[100] = "GET / HTTP/1.1\r\nHost: www.website.com\r\nConnection: close\r\n\r\n";
  	boost::tie(result, boost::tuples::ignore) = parser.parse(request_, input, input + strlen(input));
  	bool success1 = request_.method == "GET" && request_.uri == "/" && request_.http_version_major == 1 && request_.http_version_minor == 1;
  	bool success2 = request_.headers[0].name == "Host" && request_.headers[0].value == "www.website.com";
  	bool success3 = request_.headers[1].name == "Connection" && request_.headers[1].value == "close";
...
TEST_F(RequestParserTest, ParserInvalidRequest) {
  	...
  	char input[100] = "GET HTTP/1.1\r\nHost: www.website.com\r\nConnection: close\r\n\r\n";
  	boost::tie(result, boost::tuples::ignore) = parser.parse(request_, input, input + strlen(input));
  	bool success1 = request_.method == "GET" && request_.uri == "HTTP/1.1" && request_.http_version_major == 0 && request_.http_version_minor == 0;  
